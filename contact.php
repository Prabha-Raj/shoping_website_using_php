...
<body>
    ...
    <div class="container-fluid p-0">
        ...
        <div class="row p-5 bg-dark" >
            <h1 class="text-center text-light">Tell About Your Query</h1>
            <div class="col-md-6 p-5 m-auto bg-secondary rounded-4">
                <form action="" method="post" class=" w-100">
                    <label for="mnum" class="text-light" >Enter your Mobile Number : </label>
                    <input type="number" name="mnum" id="mnum" required class="form-control input-group ">
                    <label for="wnum" class="text-light">Enter your whatsapp Number : </label>
                    <input type="number" name="wnum" id="wnum" required class="form-control input-group ">
                    <label for="email" class="text-light">Enter your Email : </label>
                    <input type="email" name="email" id="email" required class="form-control input-group ">
                    <label for="subject" class="text-light">What is Subject of query : </label>
                    <input type="text" name="subject" id="subject" required class="form-control input-group ">
                    <label for="query" class="text-light">Write about your query : </label>
                    <textarea name="query" id="query" required class="form-control input-group "></textarea>
                    <input type="submit" value="Submit" name="submitquery" id="submitquery" class="btn btn-outline-warning w-100 my-3 text-center">
                </form>                
            </div>
        </div>
        <!-- fourth child end -->
...
<?php
    if(isset($_POST['submitquery'])){
        ...
        if(isset($_SESSION['username'])){
            ...
            $query = "INSERT INTO  `enquery` (user_id,mnumber,wnumber,email,querysubject,querytext,querydate) VALUES ($user_id,'$mnum','$wnum','$email','$subject','$query',NOW())";
            $run_query = mysqli_query($conn,$query);
            // echo $conn;
            if($run_query){
                echo '<script>alert("Query is Submited Successfully !")</script>';
            }else{
                echo '<script>alert("Query is Failed !")</script>';
            }
        }else{
