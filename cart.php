...
<body>
    ...
    <div class="container-fluid p-0">
        ...
        <div class="container">
            <div class="row">
                <h1 class="text-center my-2">Your Products</h1>
                <form action="" method="post">
                    <table class="table table-bordered text-center">
                        ...
                        while($rows = mysqli_fetch_array($result)){
                            ...
                            $select_crt = "SELECT * FROM `cart` WHERE product_id=$product_id";
                            $result_crt = mysqli_query($conn,$select_crt);
                            $result_row = mysqli_fetch_array($result_crt);
                            $qty = $result_row['quantity'];
                            $total = $result_row['amount'];
                            $total += $total;
                            $product_amount = $result_row['amount'];
                        ...
                        <tr class="">
                            ...
                            <td class='pt-5'><a href="./cart.php?addone=<?php echo $product_id ?>" class="nav-link " style="font-size:25px; float:right;"><i class="fa-solid fa-add"></i></a>
                            <h1 type='text' name='qty' class='form-input w-50' style="margin-left:40px;" id='qty'><?php echo $qty?></h1> 
                            <a href="./cart.php?subone=<?php echo $product_id ?>" class="nav-link" style="margin-top:-54px;font-size:35px; float:left ;"><i class="fa-solid fa-minus"></i></a>
                            </td>
                            <?php
                            $get_ip_address = get_ip_address();
                            $product_amount=($product_values*$qty);
                            if(isset($_GET['addone'])){
                                $pid = $_GET['addone'];
                                $qty += 1;
                                $product_amount=($product_values*$qty);
                                $update_cart = "UPDATE `cart` SET quantity=$qty,amount=$product_amount WHERE product_id=$pid and ip_address='$get_ip_address'";
                                $result_quantity = mysqli_query($conn,$update_cart);
                                echo "<script>open('./cart.php','_self')</script>";
                            }
                            if(isset($_GET['subone'])){
                                $pid1 = $_GET['subone'];
                                $qty -= 1;
                                $product_amount =($product_values*$qty);
                                $update_cart = "UPDATE `cart` SET quantity=$qty,amount=$product_amount WHERE product_id=$pid1 AND ip_address='$get_ip_address'";
                                $result_quantity = mysqli_query($conn,$update_cart);
                        ...
                        //    }
                          ?>
                            <td class='pt-5'><?php  echo $product_amount?>/-</td>
                            <td class='pt-5'>
                                <!-- <input type='submit' value='Update Cart' name='update_cart'
                                    class='bg-primary text-light  border-0 ' /> -->
                                <button value='<?php echo $product_id?>' name='remove_cart'
                                    class='bg-primary text-light  border-0 bg-light'><i class="text-dark fa-solid fa-trash"></i></button>
                            </td>
                        </tr>
                        <?php }
                }else{
                        ...
                        <?php
                          ...
                          if($result_count > 0){
                              while($result_c = mysqli_fetch_array($result)){
                                $price = $result_c['amount'];
                                $total += $price;
                              }
                              echo "<h4 class='px-3 m-2 '>Subtotal: <strong class='text-danger'>$total/-</strong>
                        </h4>
  ...
  global $conn;
     if(isset($_POST['remove_cart'])){
        //  foreach($_POST['removeitem'] as $remove_id){
            echo $remove_id = $_POST['remove_cart'];
